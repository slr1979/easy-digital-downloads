...
class ListTable extends List_Table {
	...
	public function get_sortable_columns() {
		return apply_filters(
			'edd_discounts_table_sortable_columns',
			array(
	...
	public function column_name( $discount ) {
		...
		if ( 'active' === strtolower( $discount->status ) ) {
			$row_actions['cancel'] = '<a href="' . esc_url(
				wp_nonce_url(
					add_query_arg(
						array(
			...
			) . '">' . __( 'Deactivate', 'easy-digital-downloads' ) . '</a>';
			...
			) . '">' . __( 'Activate', 'easy-digital-downloads' ) . '</a>';
		}

		// Archive.
		if ( 'archived' !== strtolower( $discount->status ) ) {
			$row_actions['archive'] = '<a href="' . esc_url(
				wp_nonce_url(
					add_query_arg(
						array(
			...
			) . '">' . __( 'Archive', 'easy-digital-downloads' ) . '</a>';
		}
	...
	public function column_cb( $discount ) {
		return sprintf(
			...
			'discount',
			absint( $discount->id ),
			/* translators: %s: Discount code title (name) */
			esc_html( sprintf( _x( 'Select %s', 'Noun: The discount code title (name)', 'easy-digital-downloads' ), $discount->name ) )
		);
	}
	...
	public function get_counts() {
		...
		// Ensure that 'Archived' is the last status in the status links.
	 ...
	 *
	 * @param string $which Which side of the table we're rendering, top or bottom.
	 * @since 3.1.0.4
	 */
