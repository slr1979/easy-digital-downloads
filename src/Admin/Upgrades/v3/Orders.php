...
class Orders extends Base {
	...
	public function get_data() {
		$this->maybe_reset_auto_increment();
	...
	/**
	 * Maybe reset the auto increment for the orders table.
	 *
	 * @since 3.3.0
	 * @return void
	 */
	private function maybe_reset_auto_increment() {
		if ( $this->step > 1 ) {
			return;
		}

		$last_payment = $this->get_db()->get_var(
			$this->get_db()->prepare(
				"SELECT ID
				 FROM {$this->get_db()->posts}
				 WHERE post_type = %s
		...
		// If the last payment is less than the current auto increment, we need to reset the auto increment to the last payment + 1.
		$auto_increment = $this->get_db()->get_var( "SELECT AUTO_INCREMENT FROM information_schema.TABLES WHERE TABLE_SCHEMA = '" . DB_NAME . "' AND TABLE_NAME = '" . $this->get_db()->prefix . "edd_orders'" );
		if ( $auto_increment < $last_payment ) {
			$new_auto_increment = $last_payment + 1;
			$this->get_db()->query( "ALTER TABLE " . $this->get_db()->prefix . "edd_orders AUTO_INCREMENT = " . $new_auto_increment );
		}
	}
}
