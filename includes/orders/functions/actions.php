...
function edd_add_manual_order( $args = array() ) {
	...
	if ( isset( $order_data['edd_order_address'] ) ) {
		...
		// Extract company before building the order address data.
		$company = ! empty( $address['company'] ) ? sanitize_text_field( $address['company'] ) : '';
