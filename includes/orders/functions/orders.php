...
function edd_trash_order( $order_id ) {
	...
	if ( ! empty( $trashed ) ) {
		...
		foreach ( $order_items as $item ) {
			...
			$item_trashed = $items->update_item(
				$item->id,
				array(
					'status' => 'trash',
				)
...
function edd_restore_order( $order_id ) {
	...
	if ( ! empty( $restored ) ) {
		...
		foreach ( $order_items as $item ) {
			...
			if ( ! empty( $pre_trash_status ) ) {
				$restored_item = $items->update_item(
					$item->id,
					array(
						'status' => $pre_trash_status,
					)
	...
	if ( $destroyed ) {
		...
		if ( ! empty( $items ) ) {
			foreach ( $items as $item ) {
				edd_destroy_order_item( $item->id );
			}
		}
		...
		if ( ! empty( $transactions ) ) {
			foreach ( $transactions as $transaction ) {
				edd_delete_order_transaction( $transaction->id );
			}
		}
...
function edd_build_order( $order_data = array() ) {
	...
	if ( ! empty( $order_data['cart_details'] ) && is_array( $order_data['cart_details'] ) ) {
		...
		foreach ( $order_data['cart_details'] as $key => $item ) {
			...
			if ( ! $has_tax && ! empty( $order_item_args['tax'] ) ) {
				$has_tax = true;
			}
		}
	}
	...
	if ( empty( $order_args['tax_rate_id'] ) && $has_tax ) {
	...
	// Update the order with all of the order data.
	$order = edd_get_order( $order_id );
	$order->recalculate();
