...
class HmacSignatureVerifier implements SignatureVerifierInterface
{
    ...
    public function __construct(string $secretKey, callable $signatureVerificationFailureCreator, string $signatureHeader, ?callable $templateResolver = null, string $algorithm = 'sha256', string $encoding = 'hex', ?string $signatureValueTemplate = null)
    {
        if (empty($secretKey)) {
