...
use EDD\Vendor\Carbon\Exceptions\EndLessPeriodException;
use EDD\Vendor\Carbon\Exceptions\InvalidCastException;
use EDD\Vendor\Carbon\Exceptions\InvalidIntervalException;
use EDD\Vendor\Carbon\Exceptions\InvalidPeriodDateException;
use EDD\Vendor\Carbon\Exceptions\InvalidPeriodParameterException;
use EDD\Vendor\Carbon\Exceptions\NotACarbonClassException;
use EDD\Vendor\Carbon\Exceptions\NotAPeriodException;
use EDD\Vendor\Carbon\Exceptions\UnknownGetterException;
use EDD\Vendor\Carbon\Exceptions\UnknownMethodException;
use EDD\Vendor\Carbon\Exceptions\UnreachableException;
...
class CarbonPeriod implements Iterator, Countable, JsonSerializable
{
    ...
    public static function instance($period)
    {
        ...
        if ($period instanceof self) {
            return new static($period->getStartDate(), $period->getEndDate() ?: $period->getRecurrences(), $period->getDateInterval(), $period->getOptions());
        }
    ...
    public function removeFilter($filter)
    {
        ...
        $self->filters = array_values(array_filter($this->filters, function ($tuple) use ($key, $filter) {
    ...
    public function toString()
    {
        ...
        $parts[] = $this->translate('period_interval', [':interval' => $this->dateInterval->forHumans(['join' => true])], null, $translator);
    ...
    public function __call($method, $parameters)
    {
        ...
        switch ($method) {
            case 'start':
            case 'since':
                self::setDefaultParameters($parameters, [[0, 'date', null]]);
            ...
            case 'dates':
            case 'between':
                self::setDefaultParameters($parameters, [[0, 'start', null], [1, 'end', null]]);
                return $this->setDates(...$parameters);
            case 'recurrences':
            case 'times':
                self::setDefaultParameters($parameters, [[0, 'recurrences', null]]);
                return $this->setRecurrences(...$parameters);
            case 'options':
                self::setDefaultParameters($parameters, [[0, 'options', null]]);
            ...
            case 'filters':
                self::setDefaultParameters($parameters, [[0, 'filters', []]]);
     ...
     *
     * @param \EDD\Vendor\Carbon\Carbon|\EDD\Vendor\Carbon\CarbonPeriod|\EDD\Vendor\Carbon\CarbonInterval|\DateInterval|\DatePeriod|\DateTimeInterface|string|null $date
     *
     * @return bool
     */
     ...
     *
     * @param \EDD\Vendor\Carbon\CarbonPeriod|\DatePeriod|string $period
     *
     * @return bool
     */
     ...
     * @SuppressWarnings(PHPMD.UnusedFormalParameter)
     *
     * @param \EDD\Vendor\Carbon\Carbon $current
     * @param int            $key
     *
     * @return bool|string
     */
     ...
     * @param \EDD\Vendor\Carbon\Carbon|\EDD\Vendor\Carbon\CarbonPeriod|\EDD\Vendor\Carbon\CarbonInterval|\DateInterval|\DatePeriod|\DateTimeInterface|string|null $date
     *
     * @return \EDD\Vendor\Carbon\CarbonInterface
     */
