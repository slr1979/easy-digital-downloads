...
use EDD\Vendor\Carbon\Exceptions\BadFluentConstructorException;
use EDD\Vendor\Carbon\Exceptions\BadFluentSetterException;
use EDD\Vendor\Carbon\Exceptions\InvalidCastException;
use EDD\Vendor\Carbon\Exceptions\InvalidIntervalException;
use EDD\Vendor\Carbon\Exceptions\OutOfRangeException;
use EDD\Vendor\Carbon\Exceptions\ParseErrorException;
use EDD\Vendor\Carbon\Exceptions\UnitNotConfiguredException;
use EDD\Vendor\Carbon\Exceptions\UnknownGetterException;
use EDD\Vendor\Carbon\Exceptions\UnknownSetterException;
use EDD\Vendor\Carbon\Exceptions\UnknownUnitException;
use EDD\Vendor\Carbon\Traits\IntervalRounding;
use EDD\Vendor\Carbon\Traits\IntervalStep;
use EDD\Vendor\Carbon\Traits\MagicParameter;
use EDD\Vendor\Carbon\Traits\Mixin;
use EDD\Vendor\Carbon\Traits\Options;
use EDD\Vendor\Carbon\Traits\ToStringFormat;
use Closure;
use DateInterval;
...
class CarbonInterval extends DateInterval implements CarbonConverterInterface
{
     ...
     *
     * @throws \EDD\Vendor\Carbon\Exceptions\ParseErrorException when the $interval cannot be parsed as an interval.
     *
     * @return static
     */
    public static function createFromFormat(string $format, ?string $interval)
    {
        ...
        if ($interval !== '') {
            throw new ParseErrorException('end of string', $interval);
        }
        return $instance;
    }
    /**
    ...
    public static function __callStatic($method, $parameters)
    {
        try {
            ...
            $result = static::hasMacro($method) ? static::bindMacroContext(null, function () use (&$method, &$parameters, &$interval) {
    ...
    {
        return ['years' => $this->years, 'months' => $this->months, 'weeks' => $this->weeks, 'days' => $this->daysExcludeWeeks, 'hours' => $this->hours, 'minutes' => $this->minutes, 'seconds' => $this->seconds, 'microseconds' => $this->microseconds];
    }
    /**
    ...
    public function multiply($factor)
    {
        ...
        return $this->copyProperties(static::create($yearPart)->microseconds(abs($this->totalMicroseconds) * $factor)->cascade(), true);
    }
    /**
        ...
        if (!$deep) {
            foreach ($newData as $value) {
                if ($value) {
                    ...
                    if ($value > 0 !== $positive) {
                        return $this->invertCascade($originalData)->solveNegativeInterval();
                    }
                }
            }
        }
        return $this->set($newData)->solveNegativeInterval();
    }
    /**
    ...
    {
        return $equal ? $this->greaterThanOrEqualTo($interval1) && $this->lessThanOrEqualTo($interval2) : $this->greaterThan($interval1) && $this->lessThan($interval2);
    }
    /**
    ...
    public function roundUnit($unit, $precision = 1, $function = 'round')
    {
        if (static::getCascadeFactors() !== static::getDefaultCascadeFactors()) {
            ...
            $inverted = $value < 0;
            return $this->copyProperties(self::fromString(number_format(abs($value), 12, '.', '') . ' ' . $unit)->invert($inverted)->cascade());
        }
        ...
        $this->copyProperties($next->roundUnit($unit, $precision, $function)->diffAsCarbonInterval($base));
