...
trait Date
{
    ...
    protected static function expectDateTime($date, $other = [])
    {
        ...
        if (!$date instanceof DateTime && !$date instanceof DateTimeInterface) {
            throw new InvalidTypeException($message . 'DateTime or DateTimeInterface, ' . (\is_object($date) ? \get_class($date) : \gettype($date)) . ' given');
        }
    }
    /**
     ...
     * if null given or parse the input if string given.
     *
     * @param Carbon|\EDD\Vendor\Carbon\CarbonPeriod|\EDD\Vendor\Carbon\CarbonInterval|\DateInterval|\DatePeriod|DateTimeInterface|string|null $date
     *
     * @return static
     */
    ...
    {
        return (int) static::getTranslationMessageWith(static::getTranslator(), 'first_day_of_week');
    }
    /**
    ...
    public static function getIsoUnits()
    {
        static $units = null;
        if ($units === null) {
            $units = ['OD' => ['getAltNumber', ['day']], 'OM' => ['getAltNumber', ['month']], 'OY' => ['getAltNumber', ['year']], 'OH' => ['getAltNumber', ['hour']], 'Oh' => ['getAltNumber', ['h']], 'Om' => ['getAltNumber', ['minute']], 'Os' => ['getAltNumber', ['second']], 'D' => 'day', 'DD' => ['rawFormat', ['d']], 'Do' => ['ordinal', ['day', 'D']], 'd' => 'dayOfWeek', 'dd' => function (CarbonInterface $date, $originalFormat = null) {
            ...
            }, 'SSSSSS' => ['getPaddedUnit', ['micro', 6]], 'SSSSSSS' => function (CarbonInterface $date) {
            ...
            }, 'M' => 'month', 'MM' => ['rawFormat', ['m']], 'MMM' => function (CarbonInterface $date, $originalFormat = null) {
    ...
    public function isoFormat(string $format, ?string $originalFormat = null): string
    {
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            if (preg_match('/^(LTS|LT|l{1,4}|L{1,4})/', $input, $match)) {
                ...
                $code = $match[0];
                $sequence = $formats[$code] ?? preg_replace_callback('/MMMM|MM|DD|dddd/', function ($code) {
